...
try:
    ...
    # 步骤 2: 模拟行车流量数据
    # 第一次模拟：随机范围在较小数值内
    np.random.seed(182)  # 设置随机种子以确保结果可重复
    ...
    # 第一次绘制：热力值较小
    m_small = folium.Map(location=map_center, zoom_start=12)
    HeatMap(
        heat_data_small,
        radius=15,  # 调整热力点的半径
        blur=10,    # 调整热力点的模糊程度
    ...
    output_file_small = 'baltimore_traffic_heatmap_small.html'
    m_small.save(output_file_small)
    print(f"热力图（较小热力值）已生成并保存到 {output_file_small} 文件中。")

    # 第二次绘制：热力值较大
    m_large = folium.Map(location=map_center, zoom_start=12)
    HeatMap(
        heat_data_large,
        radius=15,  # 调整热力点的半径
        blur=10,    # 调整热力点的模糊程度
    ...
    output_file_large = 'baltimore_traffic_heatmap_large.html'
